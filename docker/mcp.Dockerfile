...
COPY norintegrate-mcp/src norintegrate-mcp/src

# Build the fat JAR
RUN ./gradlew :norintegrate-mcp:bootJar -x test --no-daemon
...
COPY --from=builder /workspace/norintegrate-mcp/build/libs/*.jar app.jar
