...
| Kotlin | 2.4.10 | norintegrate-mcp only (ADR-019) |
| Spring Boot | 4.1.1 | Spring Framework 7 base. Declared in root `build.gradle.kts` |
...
Schema file: `docs/schema.sql`. The table list and database rules live in `.claude/rules/database.md`, which loads automatically when you touch `*.sql`, `*Repository.java`, or `resources/db/` files.
...
- Docs drift is checked by `scripts/check-docs.sh` (ADR indexes, relative links, version claims, merge conflict markers); enable it locally as a pre-commit hook with `git config core.hooksPath .githooks`. This is opt-in and bypassable with `--no-verify` — `.github/workflows/docs.yml` is the actual gate, the hook only speeds up local feedback.
